...
import cz.aurinet.processmaterial_dim.ui.view_models.ProcessMaterialViewModel;
...
public class ProcessMaterialActivity extends AppCompatActivity {
    ...
    private ProcessMaterialViewModel viewModel;
    private Handler handler;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_process_material);
        initViews();
        AndroidNetworking.initialize(this.getApplicationContext());
        mPlayer = MediaPlayer.create(ProcessMaterialActivity.this, R.raw.beep_err);
        singleton = ProcessMaterialPropertySingleton.INSTANCE;
        singleton.setDeviceName(Build.MANUFACTURER);
        handler = new Handler(Looper.getMainLooper());
        viewModel = new ViewModelProvider(this).get(ProcessMaterialViewModel.class);
        viewModel.getState().observe(this, uiCalls -> {
            switch(uiCalls){
                case ERR_KOD:
                    error("Chybný kód");
                    hideProgressBar();
                    showScanner();
                    break;
                case ERR_MATERIAL_NEEXISTUJE:
                    error("Neexistující blok materiálu");
                    hideProgressBar();
                    showScanner();
                    break;
                case ERR_SERVER:
    ...
    private void initViews() {
        txtBarcode = findViewById(R.id.txtBarcodeStock);
        ...
        //Nastav akci pro zmáčknutí enteru TextEditu
        ...
        txtBarcode.setHint("Načtěte/zadejte kód materiálového bloku");
    ...
    private void buttonToDefault(){
        ...
        button.setText("Načtěte kód materiálového bloku");
    ...
    private void nextActivity(){
        Intent intent = new Intent(ProcessMaterialActivity.this, ProcessMaterialSwitchActivity.class);
            ...
            public void surfaceCreated(SurfaceHolder holder) {
                try {
                    if (ActivityCompat.checkSelfPermission(getApplicationContext(), Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
                        ...
                        ActivityCompat.requestPermissions(ProcessMaterialActivity.this , new
